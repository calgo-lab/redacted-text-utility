...
def fine_tune():
    ...
    if not Path(data_dir_path / "train.txt").exists():
        with open(data_dir_path / "train.txt", "w", encoding="utf-8") as f_train:
            for _, row in tqdm(train_df.iterrows(), 
                               total=len(train_df), 
                               desc="Writing FastText format file for train split..."):
                f_train.write(convert_to_fasttext_format(row) + "\n")
    
    if not Path(data_dir_path / "dev.txt").exists():
        with open(data_dir_path / "dev.txt", "w", encoding="utf-8") as f_dev:
            for _, row in tqdm(dev_df.iterrows(),
                               total=len(dev_df), 
                               desc="Writing FastText format file for dev split..."):
                f_dev.write(convert_to_fasttext_format(row) + "\n")

    if not Path(data_dir_path / "test.txt").exists():
        with open(data_dir_path / "test.txt", "w", encoding="utf-8") as f_test:
            for _, row in tqdm(test_df.iterrows(), 
                               total=len(test_df), 
                               desc="Writing FastText format file for test split..."):
                f_test.write(convert_to_fasttext_format(row) + "\n")
